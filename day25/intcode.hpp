...
bool int_code::get_waiting_input() const {
	return waiting_input_;
}
...
int_code::int_code(const std::vector<int64_t>& ints) {
	idx_ = 0;
	relative_base_ = 0;
	ints_ = ints;
	waiting_input_ = false;
...
bool int_code::get_message(std::string& msg) {
	if (output_.size() > 0) {
		msg.clear();
		while (output_.size() > 0) {
			msg += static_cast<char>(output_.front());
			output_.pop();
		}
...
void int_code::set_command(const std::string msg) {
	for (size_t i = 0; i < msg.size(); i++)
	{
		input_.emplace(static_cast<int64_t>(msg[i]));
	}
	input_.emplace('\n');
	waiting_input_ = false;
}
